...
| `review` | findings are required before changes or approval | `spec_pack.md`, relevant standards, current diff or code | findings-first `review.md` |
| `docs` | docs or framework changes only | relevant docs, `reinforcement.md` if the docs change execution rules | updated docs plus updated `verification.md` |
| `audit` | inspect and report without changing code | relevant pack or scoped target, code, and standards | grounded `audit.md` only |
...
5. only the standards needed for the touched scope
